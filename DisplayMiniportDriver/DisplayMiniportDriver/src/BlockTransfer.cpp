...
VOID GetPitches(_In_ CONST BLT_INFO* pBltInfo, _Out_ LONG* pPixelPitch, _Out_ LONG* pRowPitch)
{
    switch(pBltInfo->Rotation)
    {
        ...
        default:
        {
            LOG_ERROR("Invalid rotation (0x%I64x) specified\n", pBltInfo->Rotation);
            *pPixelPitch = 0;
            *pRowPitch = 0;
            return;
        }
...
#pragma prefast(suppress: __WARNING_EXCEPTIONEXECUTEHANDLER, "try/except is only able to protect against user-mode errors and these are the only errors we try to catch here");
    ...
    {
        LOG_ERROR("Either dst (0x%I64x) or src (0x%I64x) bits encountered exception during access.\n", pDst->pBits, pSrc->pBits);
    }
}
...
void BltBits_Dummy(BLT_INFO* pDst, const BLT_INFO* pSrc, const UINT Color, UINT  NumRects, const RECT* pRects)
{
    ...
    __try
    {
        ...
        {
            // This is by far the most common copy function being called
            CopyBits32_32_Dummy(pDst, pSrc, Color, NumRects, pRects);
        }
        else
        {
            LOG_ERROR("BltBits_Dummy does not support generic transfers.");
        }
    }
#pragma prefast(suppress: __WARNING_EXCEPTIONEXECUTEHANDLER, "try/except is only able to protect against user-mode errors and these are the only errors we try to catch here");
    __except(EXCEPTION_EXECUTE_HANDLER)
    {
        LOG_ERROR("dst (0x%I64x) encountered exception during access.\n", pDst->pBits);
    }
}
