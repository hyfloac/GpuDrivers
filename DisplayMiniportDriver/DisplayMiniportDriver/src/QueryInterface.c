...
static void FuckyReferenceThunk(PVOID MiniportDeviceContext)
{
    (void) MiniportDeviceContext;
    LOG_WARN("FuckyReferenceThunk");
}

static void FuckyDereferenceThunk(PVOID MiniportDeviceContext)
{
    (void) MiniportDeviceContext;
    LOG_WARN("FuckyDereferenceThunk");
}

static NTSTATUS FuckyThunk0(PVOID MiniportDeviceContext)
{
    (void) MiniportDeviceContext;
...
static NTSTATUS FuckyThunk1(PVOID MiniportDeviceContext)
{
    (void) MiniportDeviceContext;
...
static NTSTATUS FuckyThunk2(PVOID MiniportDeviceContext)
{
    (void) MiniportDeviceContext;
...
NTSTATUS HyQueryInterface(IN_CONST_PVOID MiniportDeviceContext, IN_PQUERY_INTERFACE QueryInterface)
{
    ...
    if(GuidEqual(QueryInterface->InterfaceType, &GUID_DEVINTERFACE_OPM) && FALSE)
    {
    ...
    else if(GuidEqual(QueryInterface->InterfaceType, &GUID_DEVINTERFACE_OPM_2) && FALSE)
    {
    ...
    else if(GuidEqual(QueryInterface->InterfaceType, &GUID_DEVINTERFACE_OPM_2_JTP) && FALSE)
    {
    ...
    else if(GuidEqual(QueryInterface->InterfaceType, &GUID_DEVINTERFACE_OPM_3) && FALSE)
    {
    ...
    else if(GuidEqual(QueryInterface->InterfaceType, &GUID_DEVINTERFACE_I2C))
    {
        ...
        {
            DXGK_I2C_INTERFACE* pI2CInterface = (DXGK_I2C_INTERFACE*) QueryInterface->Interface;
    ...
    {
        LOG_WARN("HyQueryInterface: Querying that special fucky Interface that is required, but undocumented, and only needs to return not-implemented...\n");

        if(QueryInterface->Size == sizeof(DX_FUCKY_INTERFACE) && QueryInterface->Version == FUCKY_VERSION)
        {
            DX_FUCKY_INTERFACE* pFuckyInterface = (DX_FUCKY_INTERFACE*) QueryInterface->Interface;
